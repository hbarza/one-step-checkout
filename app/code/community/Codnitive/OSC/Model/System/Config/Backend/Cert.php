...
class Codnitive_OSC_Model_System_Config_Backend_Cert extends Mage_Core_Model_Config_Data
{
    ...
    protected function _beforeSave()
    {
        ...
        if ($value) {
            try {
                $nameSpace = Codnitive_OSC_Model_Config::getNamespace();
                ...
                if ($condition) {
                    $err = Mage::helper('osc')->__('%s extension not registered.',
                            Mage::helper('osc')->__(Codnitive_OSC_Model_Config::EXTENSION_NAME));
                ...
                $inf = array(
                    ...
                    'en' => base64_encode($crypt->encrypt((string)Codnitive_OSC_Model_Config::EXTENSION_NAME)),
                    'ev' => base64_encode($crypt->encrypt((string)Codnitive_OSC_Model_Config::EXTENSION_VERSION)),
                ...
                try {
                    ...
                    curl_setopt($ch, CURLOPT_URL, Mage::helper('osc')->getConUrl());
                ...
                if (false === $data) {
                    $err = Mage::helper('osc')->__('%s save error. Please try again later.',
                            Mage::helper('osc')->__(Codnitive_OSC_Model_Config::EXTENSION_NAME));
                    Mage::throwException($err);
                }
                if ('1' !== $data) {
                    $err = Mage::helper('osc')->__($fieldConf->label)
                        . Mage::helper('osc')->__(' or registration information for')
                        . Mage::helper('osc')->__(' %s', Mage::helper('osc')->__(Codnitive_OSC_Model_Config::EXTENSION_NAME))
                        . Mage::helper('osc')->__(' is not valid. Please check them or %s', Mage::helper('osc')->__('contact us.'));
                    Mage::throwException($err);
                }
            }
            catch (Exception $e) {
