...
//        this.accordion.disallowAccessToNextSections = true;
	...
	_onSectionClick: function(event) {
		...
		if (section.hasClassName('allow')) {
			Event.stop(event);
			this.gotoSection(section.readAttribute('id').replace('osc-', ''));
			return false;
		}
	},
	...
	setLoadWaiting: function(step, keepDisabled) {
		if (step) {
			...
			/*$('checkout-'+step+'-load')
				.insert('<div class="wait-loading" id="wait-loading"><img src="'+Codnitive_OSC.getSkinUrl()+'images/codnitive/oscheckout/wait_loading.gif"/></div>');*/
//                alert(step);
			$('osc-'+step+'-content').addClassName('show-loading');
		} /*else {
			if (this.loadWaiting) {
				$('wait-loading').remove();
			}
	...
	{
		var sectionElement = $('osc-'+section);
		sectionElement.addClassName('allow');
//        this.accordion.openSection('osc-'+section);
	...
	setBilling: function() {
		if (($('billing:use_for_shipping_yes')) && ($('billing:use_for_shipping_yes').checked)) {
			...
			$('osc-shipping').addClassName('allow');
...
//        this.accordion.openPrevSection(true);
	...
	setStepResponse: function(response){
		...
		if (response.shipping_methods_html) {
			$('checkout-'+response.shipping_methods_html.name+'-load').update(response.shipping_methods_html.html);
		}
		if (response.review_html) {
			$('checkout-'+response.review_html.name+'-load').update(response.review_html.html);
		}
		if (response.allow_sections) {
			response.allow_sections.each(function(e){
				$('osc-'+e).addClassName('allow');
			});
		}
...
//                    onSuccess: this.onSave,
	...
	updatePaymentMethods: function(updatePaymentMethodUrl){
		if (checkout.loadWaiting!=false) return;

		checkout.setLoadWaiting('payment');
		if (Codnitive_OSC.removePayment) {
			$('osc-shipping-method-content').addClassName('show-loading');
		}
		var params = Form.serialize(this.form);
		params += '&'+Form.serialize(shipping.form);
...
//		shippingMethod.updateReview(Codnitive_OSC.updateReviewShippingUrl);
		if (transport && transport.responseText){
			try{
		...
		Codnitive_OSC.selectPaymentMethod();
		shippingMethod.save();
//		Codnitive_OSC.saveShippingMethod();
		$('osc-shipping-method-content').removeClassName('show-loading');
		$('osc-payment-content').removeClassName('show-loading');
		// DELETE
...
//                    onSuccess: this.onSave,
	...
	saveForReviewUpdate: function(){
		if (checkout.loadWaiting!=false) return;
//		var validator = new Validation(this.form);
//		if (validator.validate()) {
////            checkout.setLoadWaiting('shipping-method');
			var request = new Ajax.Request(
				Codnitive_OSC.saveReviewShippingMethodUrl,
				{
					method:'post',
//                    onComplete: this.onComplete,
//                    onSuccess: this.onSave,
...
//		}
	...
	updateShippingMethods: function(updateShippingMethodUrl){
		if (checkout.loadWaiting!=false) return;

		checkout.setLoadWaiting('shipping-method');
//        var form = Codnitive_OSC.isShippingSameAsBilling() ? 'co-billing-form' : 'co-shipping-form'
		var params = Form.serialize(this.form);
		params += '&'+Form.serialize(billing.form);
		params += '&'+Form.serialize(shippingMethod.form);
		params += '&'+Form.serialize(payment.form);
		var request = new Ajax.Request(
...
ShippingMethod.prototype = {
	...
	save: function(){
		...
		if (checkout.loadWaiting!=false) return;
		Codnitive_OSC.selectShippingMethod();
		if (this.validate()) {
...
//			params += '&'+Form.serialize(shipping.form);
	...
	updateReview: function(updateReviewShippingUrl){
		if (checkout.loadWaiting!=false) return;
		checkout.setLoadWaiting('review');
		var params  = Form.serialize(this.form);
		params += '&'+Form.serialize(billing.form);
...
//        payment.updateReviewPayment(Codnitive_OSC.updateReviewPaymentUrl);
	},

	resetLoadWaiting: function(transport){
		checkout.setLoadWaiting(false);
//        $('wait-loading').remove();
		$('osc-review-content').removeClassName('show-loading');
	},
...
//                    onSuccess: this.onSave,
	...
	updateReview: function(updateReviewPaymentUrl){
		if (checkout.loadWaiting!=false) return;
		checkout.setLoadWaiting('review');
		var params  = Form.serialize(this.form);
		params += '&'+Form.serialize(billing.form);
...
//        $('wait-loading').remove();
	...
	nextStep: function(transport){
		...
		*/
		...
		/*if (response.shipping_methods_html) {
			$('checkout-shipping-method-load').update(response.shipping_methods_html);
		}*/
...
Review.prototype = {
	...
	save: function(){
		var validateForms = Codnitive_OSC.validation();
		if (!validateForms) {
			return;
		}

		var saveForms = Codnitive_OSC.save();
		if (saveForms != false) {
			if (checkout.loadWaiting!=false) return;
			checkout.setLoadWaiting('review');
